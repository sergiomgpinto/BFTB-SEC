...
public class Account {
    ...
    public String getPublicKeyString() { return _publicKeyString; }
    ...
    public synchronized void removePendingTransaction(String senderKey, int transactionId){
        ...
        _pending.removeIf(pending -> pending.getTransactionId() == transactionId && senderKey.equals(pending.getSenderKey()));
    }
    ...
    public synchronized void addPending(String publicKey, int amount, boolean isSender, int transactionId) {
        if (isSender) {
            _pending.add(new Pending(publicKey, amount, TransactionStatus.PENDING, TransactionType.WITHDRAWAL,transactionId));

        } else {
            _pending.add(new Pending(publicKey, amount, TransactionStatus.PENDING, TransactionType.CREDIT,transactionId));
